...
--
-- Bundle das 5 migrations, geradas a partir de packages/db/migrations/.
...
--
-- Tinhamos dois. `mission` em mkt_v2.agent_registry, e `uncertainty` (mais o
...
--
-- `mkt_v2.agent_runs.prompt_version` existe desde a 0005 e nunca foi escrita.
...
-- ── Para reverter ───────────────────────────────────────────────────────
--
--   alter table mkt_v2.agent_runs drop column persona_version;
--   drop table mkt_v2.agent_personas;
-- =====================================================================

create table mkt_v2.agent_personas (
...
create unique index agent_personas_one_active
  on mkt_v2.agent_personas (agent_id) where status = 'ACTIVE';
...
-- na mao e sem policy.
alter table mkt_v2.agent_personas enable row level security;

-- O trace precisa saber com que persona o run falou.
alter table mkt_v2.agent_runs add column persona_version integer;

comment on column mkt_v2.agent_runs.prompt_version is
...
-- ---------------------------------------------------------------------
insert into mkt_v2.agent_personas
...
begin
  select array_agg(a.agent_id) into orfaos
    from mkt_v2.agent_registry a
   where a.status = 'ACTIVE'
     and not exists (
       select 1 from mkt_v2.agent_personas p
...
-- ── Para reverter ───────────────────────────────────────────────────────
--
--   drop index mkt_v2.agent_registry_one_active, mkt_v2.capability_registry_one_active;
--   alter table mkt_v2.rule_policies
...
comment on column mkt_v2.rule_policies.reason is
...
comment on column mkt_v2.rule_policies.expires_at is
...
create index rule_policies_contencao
  on mkt_v2.rule_policies (org_id, created_at desc)
...
create unique index agent_registry_one_active
  on mkt_v2.agent_registry (agent_id) where status = 'ACTIVE';

create unique index capability_registry_one_active
  on mkt_v2.capability_registry (capability_id) where status = 'ACTIVE';
