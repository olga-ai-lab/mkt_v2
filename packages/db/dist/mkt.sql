...
alter table mkt.schema_migrations enable row level security;
insert into mkt.schema_migrations (name) values
  ('0001_iam.sql'),
  ('0002_brand_content.sql'),
  ('0003_publishing.sql'),
  ('0004_governance.sql'),
...
--   select count(*) from information_schema.tables where table_schema = 'mkt';
--   select capability_id, status from mkt.capability_registry order by 1;
